...
from user.models import User, Guia, Turista, Especialidade, Avaliacao, Pais, Estado, Cidade, Tipo

SPECIALITIES = [["Tradução", ['acompanhante', 'chat']], ["Direção", ['acompanhante']],
                ["Assistência a Pessoas com Deficiência", ['acompanhante']], ["Cultura", ['chat', 'acompanhante']],
                ["Aventura", ['chat', 'acompanhante']], ["Ecoturismo", ['chat', 'acompanhante']],
                ["Praia", ['chat', 'acompanhante']], ["Baladas", ['chat']]]
GENDERS = ("male", "female")
...
print("Criando Tipos")
Tipo.objects.create(descricao='chat')
Tipo.objects.create(descricao='acompanhante')

print("Criando especialidades...")
espec_list = []
for espec in SPECIALITIES:
    espec_obj = Especialidade.objects.create(descricao=espec[0])
    for item in espec[1]:
        tipo = Tipo.objects.get(descricao=item)
        espec_obj.tipo.add(tipo)
    espec_obj.save()
